...
function MeshVisualization() {
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      ...
      />
      <svg viewBox={`0 0 ${svgWidth} ${svgHeight}`} className="relative z-10 w-full max-w-[180px] sm:max-w-[360px] h-auto">
        <defs>
        ...
        {CONNECTIONS.map(([from, to], i) => {
          ...
          return (
            <motion.line
              ...
              animate={{ pathLength: 1, opacity: [0.2, 0.5, 0.2], strokeDashoffset: [0, -40] }}
              transition={{
      ...
      {[
        ...
        <motion.div
          key={i}
          className="absolute text-[9px] sm:text-[10px] font-mono text-primary/70 border border-primary/20 bg-primary/5 rounded px-1.5 py-0.5 backdrop-blur-sm"
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl 2xl:max-w-[1600px] mx-auto px-4 sm:px-8 py-4 sm:py-8 md:py-10 relative z-10 w-full">
        {/* Mobile layout: side-by-side text + compact viz */}
        <div className="flex flex-col sm:hidden gap-4">
          ...
          >
            ...
            <h2 className="text-3xl font-black leading-[0.9] tracking-tight" data-testid="text-meshtastic-title">
          ...
          <div className="grid grid-cols-2 gap-2">
            {features.map((feat, i) => (
              <motion.div
                key={feat.title}
                className="flex items-center gap-2 p-2.5 rounded-xl border border-primary/10 bg-primary/5"
          ...
          <Link href="/resilience-layer">
            <Button size="lg" variant="outline" className="group w-full text-sm" data-testid="button-resilience-layer">
